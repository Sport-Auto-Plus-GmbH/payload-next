...
export async function up({ db }: MigrateUpArgs): Promise<void> {
  // Fresh environments (a new developer's machine, CI) don't have the
  // dedicated "payload" schema yet — create it before anything else. Kept
  // idempotent so re-running against an environment that already has it
  // (e.g. one bootstrapped via dev-mode schema push) is still safe.
  await db.execute(sql`CREATE SCHEMA IF NOT EXISTS "payload";`)

  await db.execute(sql`
   CREATE TYPE "payload"."enum_users_roles" AS ENUM('super-admin', 'user');
  CREATE TYPE "payload"."enum_users_tenants_roles" AS ENUM('tenant-admin', 'tenant-viewer');
  ...
  CREATE TABLE "payload"."users_tenants_roles" (
  	"order" integer NOT NULL,
  	"parent_id" varchar NOT NULL,
  	"value" "payload"."enum_users_tenants_roles",
  	"id" serial PRIMARY KEY NOT NULL
  );
  ...
  ALTER TABLE "payload"."users_tenants_roles" ADD CONSTRAINT "users_tenants_roles_parent_fk" FOREIGN KEY ("parent_id") REFERENCES "payload"."users_tenants"("id") ON DELETE cascade ON UPDATE no action;
  ...
  CREATE UNIQUE INDEX "tenants_domain_idx" ON "payload"."tenants" USING btree ("domain");
  ...
  CREATE INDEX "users_tenants_roles_order_idx" ON "payload"."users_tenants_roles" USING btree ("order");
  CREATE INDEX "users_tenants_roles_parent_idx" ON "payload"."users_tenants_roles" USING btree ("parent_id");
...
export async function down({ db }: MigrateDownArgs): Promise<void> {
  ...
  DROP TABLE "payload"."users_tenants_roles" CASCADE;
  ...
  DROP TYPE "payload"."enum_users_tenants_roles";`)
}
